...
function start() {
  ...
  preloadImages().then(() => {
    ...
    document.querySelectorAll('.sort.username').forEach(el => el.style.display = 'block');
    display();
  });
}
...
function display() {
  ...
  document.querySelector('.left.sort.username').innerHTML = charNameDisp(leftChar.username);
  document.querySelector('.right.sort.username').innerHTML = charNameDisp(rightChar.username);
...
function result(imageNum = 20) {
  ...
  document.querySelectorAll('.sort.username').forEach(el => el.style.display = 'none');
